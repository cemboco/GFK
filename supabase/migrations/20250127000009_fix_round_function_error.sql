/*
  # Fix ROUND Function Error in calculate_gfk_level Function

  Problem: "function round(double precision, integer) does not exist"
  Solution: Use proper PostgreSQL casting and ROUND function syntax
*/

-- Fix the calculate_gfk_level function with correct ROUND usage
...
BEGIN
  ...
  SELECT 
    ...
    CASE 
      WHEN COUNT(m.id) > 0 THEN 
        (COUNT(m.id)::NUMERIC / GREATEST(DATE_PART('day', NOW() - MIN(m.created_at)), 1))::DECIMAL(5,2)
      ELSE 0::DECIMAL(5,2)
  ...
  SELECT 
    ...
    COALESCE(user_stats.avg_daily_usage, 0::DECIMAL(5,2)),
    ...
    CASE WHEN include_stats THEN
      jsonb_build_object(
        ...
        'activity_metrics', jsonb_build_object(
          'daily_avg', COALESCE(user_stats.avg_daily_usage, 0::DECIMAL(5,2)),
          'weekly_trend', CASE 
